...
class ParserAlvitek2 extends Command {
     ...
     * @var string
     */
    protected $signature = 'parser:alvitek2';
...
//            \DB::table('products')->where('provider_id', '=', $this->provider_id)->where('checked', '=', 1)->update(['enabled'=>1]);
            \Storage::disk('parser')->delete($this->fname);
    ...
    protected function download_xml_file()
    {
        $config = [
            'base_uri' => 'http://alvitek.com.ru/',
            'cookies' => true,
            'headers' => [
        ...
        $guzzle->post('auth/', [
                'form_params' => [
                    ...
                    'TYPE' => 'AUTH',
                    'backurl' => '/',
                    'USER_LOGIN' => "9365544@mail.ru",
                    'USER_PASSWORD' => "Пуговичников",
                    'USER_REMEMBER' => 'Y'
                ]
            ]
        );

        $page = $guzzle->get('personal/last/')->getBody()->getContents();
        preg_match('/bitrix_sessid\':\'(.+)\'/', $page, $matches);
        $dt = [
            "ACTION" => "startExport",
            "DATA"=> ["PROFILE"=>967],
            "CLASS"=>"Alvitek\\Partners\\Export\\ExportFile"
        ];

        $sessid = $matches[1];

        $page = $guzzle->post('local/components/al/export.catalog/ajax.php?sessid='.$sessid, [
                'headers'=>array('Content-Type'=>'application/json'),
                'json' => $dt

            ]
        )->getBody()->getContents();
        $json = \GuzzleHttp\json_decode($page);
        $steps = $json->DATA->PROCESS->steps;
        $file = $json->DATA->FILE->LOCAL_PATH;
        ...
        $guzzle->get($file, [
            'sink' => \Storage::disk('parser')->path($this->fname)
        ]);
    }
